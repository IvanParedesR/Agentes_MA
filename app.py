...
# Reemplaza la configuración actual de pytesseract con esto:
import pytesseract
...
st.set_page_config(page_title="Merger and Acquisition Analysis", layout="wide", page_icon="📄")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("Tarea de Agentes AI - ITAM 2025")
    ...
    if st.button("Analizar Poder Legal"):
        with st.spinner("Extrayendo información del poder..."):
            ...
            try:
                ...
                cols = st.columns(2)
                with cols[0]:
                    st.metric("🧑 Otorgante", data["otorgante"])
                    st.metric("👤 Apoderado", data["apoderado"])
                with cols[1]:
...
tab1, tab2, tab3 = st.tabs(["Depósitos Bancarios", "Fusiones y Adquisiciones", "Poderes Legales"])
